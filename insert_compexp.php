...
// Choose table
$query="SELECT * FROM BrownieComputerExpert";
...
if($tempCount > $eventMax-$count){
	for($i=0; $i<10;$i++)
	{
		if(!empty($_POST['firstname'.$i]))
		{
			...
			$sql = "INSERT INTO WaitlistBrownieCompExp (troopNumber, firstName,
			lastName, dob, city)
	...
	for($i=0; $i<10;$i++)
	{
		if(!empty($_POST['firstname'.$i]))
		{
			...
			$sql = "INSERT INTO BrownieComputerExpert (troopNumber, firstName,
			lastName, dob, city)
			VALUES ('$troopnumber','$firstname','$lastname','$dob','$city');";
