...
// Choose table
$query="SELECT * FROM JuniorScienceDiscovery";
...
if($tempCount > $eventMax-$count){
	for($i=0; $i<10;$i++)
	{
		if(!empty($_POST['firstname'.$i]))
		{
			...
			$sql = "INSERT INTO WaitlistJuniorSciDisc (troopNumber, firstName,
			lastName, dob, city)
	...
	for($i=0; $i<10;$i++)
	{
		if(!empty($_POST['firstname'.$i]))
		{
			...
			$sql = "INSERT INTO JuniorScienceDiscovery (troopNumber, firstName,
			lastName, dob, city)
			VALUES ('$troopnumber','$firstname','$lastname','$dob','$city');";
...
mysql_close($con);
header("Refresh:5; Location: index.php#form2");
ob_flush();
?>
