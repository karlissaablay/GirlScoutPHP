...
if($tempCount > $eventMax-$count){
	for($i=0; $i<10;$i++)
	{
		if(!empty($_POST['firstname'.$i]))
		{
			...
			$sql = "INSERT INTO WaitlistJuniorAero (troopNumber, firstName,
			lastName, dob, city)
	...
	echo "Your total amount of scouts exceeds the max amount allowed in event. Scouts added to waitlist. Email has been sent.";
}
else{
	for($i=0; $i<10;$i++)
	{
		if(!empty($_POST['firstname'.$i]))
		{
			...
			$sql = "INSERT INTO JuniorAerospace (troopNumber, firstName,
			lastName, dob, city)
			VALUES ('$troopnumber','$firstname','$lastname','$dob','$city');";
	...
	echo "Scouts added to event. Email has been sent.";
}
