...
// Choose table
$query="SELECT * FROM BrownieHomeScientist";
...
if($tempCount > $eventMax-$count){
	for($i=0; $i<10;$i++)
	{
		if(!empty($_POST['firstname'.$i]))
		{
			...
			$sql = "INSERT INTO WaitlistBrownieHomeSci (troopNumber, firstName,
			lastName, dob, city)
	...
	for($i=0; $i<10;$i++)
	{
		if(!empty($_POST['firstname'.$i]))
		{
			...
			$sql = "INSERT INTO BrownieHomeScientist (troopNumber, firstName,
			lastName, dob, city)
			VALUES ('$troopnumber','$firstname','$lastname','$dob','$city');";
